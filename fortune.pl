...
sub event_privmsg {
    ...
    if (($INTERVAL > 0) && ($createtimer==1)) {
    	...
    	undef $timer{$tname};
        $timer{$tname}=AnyEvent->timer (after => $INTERVAL, interval => $INTERVAL, cb => sub { spit_fortune($server, $target); });
        Irssi::print $tname." - created/restarted timer with $INTERVAL second interval";
    }
